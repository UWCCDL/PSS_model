...
noncom <- read.table(unzip("sims-noncompetitive/sims-positive-negative-alpha-egs-noncomp.zip"), 
                   header=T,
                   sep=","
                   )
...
data2 <- read.table(unzip("sims-noncompetitive-reverse/sims-positive-negative-alpha-egs-noncompetitive-reverse.zip"), 
                    header=T,
                    sep=","
)
...
a_data2 <- merge(a_data1, a_data2, all=T)

a_data2 <- subset(a_data2, a_data2$Alpha < 0.1)
a_data2 <- subset(a_data2, a_data2$EGS > 0)


s_data2$Pattern <- vpattern(s_data2$ChooseA, s_data2$AvoidB)

psp2 <- tapply(s_data2$ChooseA, s_data2$Pattern, length)
